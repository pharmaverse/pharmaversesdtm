...
#'
#' It reads metadata from a JSON file ('inst/extdata/sdtms-specs.json') and uses that information to create R documentation files
#' for each dataset. The script utilizes helper functions to handle metadata retrieval and format the documentation properly.
#' The generated documentation files are saved in the 'R' directory and should not be manually edited, as they are subject to automatic regeneration.

# Load the necessary library for JSON
library(jsonlite)
...
for (dataset_name in datasets) {
  ...
  # Retrieve metadata from the specs JSON
